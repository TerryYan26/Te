...
void print_date_stats(node **table, unsigned long size, char *datestr) {
  ...
  for (i = 0; i < size; i++) {
    ...
    while (curr != NULL) {
      if (curr->year == year && curr->month == month && curr->day == day) {
		...
		if(min_temp > curr->temp){
			min_temp = curr->temp;
		}
		if(max_temp < curr->temp){
			max_temp = curr->temp;
		}
  ...
  if (total_count == 0) {
	 fprintf(stderr, "Unable to find any data for the date %s.\n",datestr);
	...
	printf("Minimum pm2.5: %d\tMaximum pm2.5: %d\tAverage pm2.5: %d\n",
		 min_pm, max_pm, (total_pm25 / total_count));
	printf("Minimum temp: %d\tMaximum temp: %d\tAverage temp: %d\n",
		 min_temp, max_temp, (total_temp / total_count));
...
int load_table(node **table, unsigned long size, char *filename) {
  ...
  while (fgets(line, MAX_LINE_LENGTH, fp) != NULL) {
    ...
    unsigned long hash_val = hash(datestr) % size_read;
...
void print_info(node **table, unsigned long size) {
  ...
  */
  ...
  for (i = 0; i < size; i++) {
    ...
    if (chain_length < min_chain && chain_length >0)
